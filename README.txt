...
дальше можно так: (помним, что бд в PostgreSQL в докере)
    ...
    - в GET запрос http://localhost8080/api/timesheets/1 добавить токен
...
UPD:
   - ключ прямо в коде - просто для наглядности, что это такое
   - по поводу поведения аспекта, когда аннотации @Recover нет, в блоке else забыл коммент добавить;
        поправил, новая ошибка была просто для наглядности и другого сообщения


HW9
Микросервисы
сделано 3 модуля (фронт, бэк, эврика - как централизация связей)

Проблема с показом страниц (что спрашивал в чате) решил - сделаны доп обращения к бэку

Браузерное обращение по http://localhost:1234/{..}
тк оставлена зависимость security, кинет на форму логина, там стандартно (user, пароль в консоли)
были мысли сделать передачу и обработку токена, но реализовать толком не вышло, не стал оставлять

Лишние файлы не подчищал
 Скорее всего потом верну монолит, микросервисы оставлю в отдельной ветке, может что-то преобразую, не знаю еще


HW10 в ветке MAIN

HW11
Создание новых модулей и их подключение как стартеров в микросервисы
Последовательность для инфы в logging-starter INFO.txt

В Logging аспекте есть вариант с глобальной настройкой печати аргументов (вкл/выкл весь аспект)
А также оставил возможность указывать настройку логирования в условии, настройки параметра внутри аннотации
Т.е, может быть, что модуль включен, но указали, что не печатать аргументы
    == показываем, что аннотация включена не печатаем аргументы;
А может быть, что модуль всецело выключен, тогда ни о чем не предупреждаем, просто тихо делаем код и все


Recover аспект сделан отдельным модулем, как starter
Также добавлены:
 - файл авто конфигурации, как с логгером
 - файл properties, для получения настроек из .yaml
 - обновлены параметры аннотации и реализация
 - файл автоконфига в recover
 - зависимость в rest

В .yaml можно указывать
    - включение/выключение всего аспекта
    - список классов, которые будут игнорироваться
Также, в самой аннотации есть возможность выбора объединять ли списки классов
    или использовать только те, что указаны в аннотации

Пример обоих стартеров показан в TimesheetService модуля timesheet-rest
в методах findById() и findAll()

    (В findById() происходит выброс ошибки, которая указана в .yaml для моделирования ситуации,
        в консоли увидим, что такая ошибка не обрабатывается).

Также выключен ExceptionHandler, который перехватил бы моделируемую ситуацию, сделано для демонстративного примера

Обновлены поля фиксми для этого задания, вроде все описал, что надо
