...
HW10
Тесты TimesheetController ---->>>>  src/test/java/gb/pract/timesheet/controller/TimesheetControllerTest.java
Остальное семинарские
Проверка на что хватило фантазии, прекрасно понимаю, что там можно более сложные схемы построить для проверок

Вроде бы тема понятна, особенно с учетом того, что пишу их впервые;

В тестах также учтено, что запросы требуют авторизации (хотя можно было поднять контекст без секурити (ведь так?)),
    там добавлены хедеры и генерация токена (на всякий он спамится в консоли)

Ситуация с Mock пока сложна, если правильно понимаю,
    ими пользуемся только когда поднимаем не весь контекст, а определенные бины (из аннотации над тестом)
